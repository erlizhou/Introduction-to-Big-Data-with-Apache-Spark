...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    for rating in ratings:
        sum += rating
        count += 1
    return (movieID, (count, sum / count))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD)).map(lambda (a, b): (a, (b[0] - b[1]) ** 2))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (a, b): (a, b[0])).filter(lambda (a, b): b > 75)
