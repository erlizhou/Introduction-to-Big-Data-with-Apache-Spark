...
def simpleTokenize(originalString):
    ...
    """
    splitString = re.split(split_regex, originalString)
    lowerString = map(lambda x: x.lower(), splitString)
    removeString = [s.translate(string.maketrans("", ""), string.punctuation.replace("_", "")) for s in lowerString]
    return filter(None, removeString)
...
def tokenize(string):
    ...
    """
    simpleString = simpleTokenize(string)
    simpleString = [x for x in simpleString if x not in stopwords]
    return simpleString
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda (a, b): len(b)).sum()
...
def findBiggestRecord(vendorRDD):
    ...
    """
    sortedVendorRDD = vendorRDD.sortBy(lambda x: len(x[1]), ascending = False)
    return sortedVendorRDD.take(1)
...
def tf(tokens):
    ...
    tokenMapping = dict()
    for t in tokens:
        if t in tokenMapping:
            tokenMapping[t] += 1
        else:
            tokenMapping[t] = 1

    for k in tokenMapping:
        tokenMapping[k] /= float(len(tokens))
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.map(lambda (a, b): list(set(b)))
    flatUniqueTokens = uniqueTokens.flatMap(lambda a: a)
    tokenCountPairTuple = flatUniqueTokens.map(lambda a: (a, 1))
...
avgSimDups = trueDupsRDD.map(lambda (a, (b, c)): b).sum() / trueDupsCount

nonDupsRDD = (sims
             .subtractByKey(goldStandard))

avgSimNon = nonDupsRDD.map(lambda (a, b): b).sum() / nonDupsRDD.count()
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda (a, b): (a, tfidf(b, idfsFullWeights)))

googleWeightsRDD = googleFullRecToToken.map(lambda (a, b): (a, tfidf(b, idfsFullWeights)))
...
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(swap).groupByKey().mapValues(list)
                .cache())
...
def fastCosineSimilarity(record):
    ...
    value = s / norm(amazonWeightsBroadcast.value[amazonRec]) / norm(googleWeightsBroadcast.value[googleRec])
    key = (amazonRec, googleRec)
    return (key, value)

similaritiesFullRDD = (commonTokens
                       .map(lambda (a): fastCosineSimilarity(a))
                       .cache())
